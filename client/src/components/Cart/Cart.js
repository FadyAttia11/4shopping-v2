...
const Cart = () => {
    ...
    const displayTotalForProduct = (product) => {
        if(quantity.length == products.length) {
            const currentElementTotal = totalForProduct.filter((element) => element.productId == product._id)
    ...
    const removeProduct = async (product) => {
        setProducts([])
        setCart(cart.filter((cartElement) => cartElement.productId !== product._id))
        const currentCartElement = cart.filter((cartElement) => cartElement.productId == product._id)
